...
void IRAM_ATTR next_station() {
    ...
    log_message(
        formatString("Next: %s -> %s", stations[oldStation].name.c_str(), stations[currentStation].name.c_str()));
...
void IRAM_ATTR previous_station() {
    ...
    log_message(
        formatString("Prev: %s -> %s", stations[oldStation].name.c_str(), stations[currentStation].name.c_str()));
    ...
    switch (audioState) {
        ...
        case AUDIO_CONNECTING: {
            if (millis() - audioStateTime > AUDIO_STATE_DELAY) {
                ...
                if (url.indexOf("-aac-") >= 0 || url.indexOf(".aac") >= 0 || url.indexOf("/aac") >= 0 ||
                    url.indexOf("aac=") >= 0) {
...
void process_audio_data_for_visualizer(const int16_t* data, int len) {
    ...
    for (int i = 0; i < VISUALIZER_BANDS; i++) {
        ...
        if (actual_band_size > 0) {
            ...
            visualizerBands[i] =
                constrain(map(avg_amplitude, 0, VISUALIZER_MAX_AMPLITUDE, 0, SCREEN_HEIGHT), 0, SCREEN_HEIGHT);
        } else {
            visualizerBands[i] = 0;
        }
    }
